...
- [ ] F10 - BookingImpl - many users should be able to book class<br>Implementation:<br>- when creating booking, user should have the option to allow booking to be shareable or not. isShareable boolean added to booking object<br>- create b ooking ednpoint will take just an id? and then service will turn into a list or fe will send lsit of users - with only one user?<br>- new endpoint which is used for user to book already created bookings such as 'book shared booking'<br>- booking will have <List> Users instead of User<br>- booking will use @ManyToMany -> no cascading<br>- will have a join table<br>- where user is referenceed from the booking, the implementation will have to be changed<br>- update repository query for find all by user id<br>- update tests<br><br><br>Reconsider this implementation of booking booking. Including shared booking. There can only be 3 shared <br>chefck the normal query for bookings which shareable is false.<br>check query again for bookings which shareable is true. if there is shareable bookings here, then shareable has to be true. if there is bookings here also make sure there is no more than 2 bookings present.
- [ ] CR solution:<br>- [x] in Booking class, add @ManyToMany rs to List<user><br>- [x] update constructor booking<br>- [x] add boolean for isShareable.<br>- [x] add boolean is shareable in booking request class<br>- [x] in service layer, add user to list and then save the booking <br>- [x] update tests<br><br>- [ ] new endpoint to book onto already created bookings. will need user id and booking id.<br>- [ ] service must check again if booking isShareabl<br><br>- [ ] update repository for find all by user id<br><br>- [ ] cancel the booking. cancel booking only if there no more users in the booking, otherwise just remove the users.<br>- [ ] if user is already on booking -> throw error saying user has already booked booking!<br>- [ ] only up to three users can book to booking<br>- [ ] ask jope if user can do different class stype such as practice private -> only private lessons can share<br>- [ ] bookings can be booked different time within th etime.... this makes things more ocmplicated os whole logic will have to be changed
- [ ] Price model which holds booking id, total price and then the dance class also which is sued inside booking?
- [ ] UPDATED REQUIREMENTS<br>-> booking can be shareable betweem users. Up to 3<br>- each teacher will pay the same amount for the booking for example. if teacher books a room for 60 and a teacher wants to share the room its the same for 60.<br>- price is actually one fixed price not different price per 30/45/60 mins. ![[Pasted image 20240509204851.png]]
